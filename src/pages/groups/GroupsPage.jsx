...
export default function GroupsPage() {
  ...
  const open = (type, item = null) => {
    ...
    setForm(type === 'create' ? { ...emptyMember, name: '', monthly_basic_points: '' }
  ...
  const submit = async (event) => {
    ...
    await run(async () => {
      ...
      if (type === 'create') {
        const saved = await createGroup({ ...optionalFields(form), monthly_basic_points: Number(form.monthly_basic_points) });
  ...
  return <Stack spacing={3}>
    ...
    <Dialog open={!!modal} onClose={() => { if (!busy) setModal(null); }} fullWidth maxWidth="sm"><Stack component="form" onSubmit={submit}>
      ...
      <DialogContent><Stack spacing={2} sx={{ pt: 1 }}>
        ...
        {['create', 'add'].includes(modal?.type) && field('employee_number', modal?.type === 'create' ? '리더 사번' : '사번', { required: true, inputProps: { maxLength: 50 }, helperText: '기본 비밀번호로 생성되며 첫 로그인 시 변경해야 합니다.' })}
        ...
        {modal?.type === 'reset' && <Typography>{modal.item.employee_number} 계정을 기본 비밀번호로 초기화합니다. 기존 로그인은 종료되며 다음 로그인 시 변경해야 합니다.</Typography>}
